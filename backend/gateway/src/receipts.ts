...
class ReceiptStore {
  ...
  async init(): Promise<void> {
    try {
      ...
      console.log(`[RECEIPTS] Loaded ${this.receipts.length} receipts from disk`);
    } catch (error) {
      // File doesn't exist yet, start with empty array
      console.log("[RECEIPTS] Starting with empty receipt store");
      this.receipts = [];
  ...
  async save(receipt: Receipt): Promise<void> {
    ...
    await this.persist();
    console.log(`[RECEIPTS] Saved receipt ${receipt.id} for ${receipt.method}`);
  }
